@extends('layouts.app')
@section('title', 'بيع مباشر')
@section('page-title')<span>⚡ بيع مباشر</span>@endsection
...
<form action="{{ route('quick-sale.store') }}" method="POST" id="qsForm">
@csrf
<div class="row g-3">

    {{-- ===== العمود الأيمن: المنتجات + السلة ===== --}}
    <div class="col-lg-8">
        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="text-muted small" id="productsCount">{{ $products->count() }} منتج</span>
            </div>
                ...
                <div class="row g-2" id="productsGrid">
                    ...
                    <div class="col-6 col-md-4 col-xl-3 product-card-wrap"
                        ...
                        <div class="qs-product-card h-100"
                             onclick="addToCart({{ $p->id }}, '{{ addslashes($p->name) }}', {{ $p->unit_price }}, {{ $p->tax_rate }}, '{{ $p->unit }}')">
                            <div class="p-name mb-1">{{ $p->name }}</div>
                            <div class="p-price">{{ number_format($p->unit_price, 2) }} {{ $defaultCurrency }}</div>
                            <div class="d-flex justify-content-between align-items-center mt-1">
                                <span class="p-stock">{{ $p->unit }}</span>
                                <span class="badge {{ $stockClass }}" style="font-size:.65rem">
        ...
        <div style="position:sticky;top:80px">
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body py-2">
                    ...
                    <select name="customer_id" class="form-select form-select-sm mb-2" id="customerSelect" onchange="onCustomerChange(this.value)">
                        ...
                        <option value="{{ $c->id }}" {{ $c->id == $cashCustomerId ? 'selected' : '' }}>
                            {{ $c->name }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="total-box mb-3">
                ...
                <div class="border-top border-white border-opacity-25 pt-2">
                    ...
                    <div class="val" id="totalDisp">0.00 {{ $defaultCurrency }}</div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body py-2">
                    ...
                    <div class="d-flex flex-wrap gap-2" id="paymentMethodBtns">
                        ...
                        <button type="button" class="pm-btn {{ $loop->first ? 'selected' : '' }}"
                                data-code="{{ $pm->code }}"
                                onclick="selectPaymentMethod('{{ $pm->code }}', this)">
                            {{ $pm->name }}
                        </button>
                        @endforeach
                    ...
                    <input type="hidden" name="payment_method" id="paymentMethodInput"
                           value="{{ $paymentMethods->first()?->code ?? 'cash' }}">

                    {{-- المبلغ المستلم (يظهر فقط لغير الآجل) --}}
...
{{-- حقول مخفية للمخزن --}}
@if($userWarehouses->count() > 1)
<input type="hidden" name="warehouse_id" id="warehouseIdInput" value="{{ $userWarehouse?->id }}">
@endif
...
let currentWarehouseId  = {{ $userWarehouse?->id ?? 'null' }};
